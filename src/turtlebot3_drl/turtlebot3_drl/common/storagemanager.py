...
#         self.model_dir = self.machine_dir
...
#     def network_save_weights(self, network, model_dir, stage, episode, name):
...
#         filepath = os.path.join(model_dir, name + '_stage' + str(stage) + '_episode' + str(episode) + '.pt')
#         print(f"saving {name} model for episode: {episode}")
#         torch.save(network.state_dict(), filepath)
...
#             self.network_save_weights(network, self.session_dir, self.stage, episode, name) #add name
...
#     def store_model(self, model, as_pickle=True):
#         if as_pickle:
#             with open(os.path.join(self.session_dir, f'stage{self.stage}_agent.pkl'), 'wb') as f:
#                 pickle.dump(model, f, pickle.HIGHEST_PROTOCOL)
#         else:
#             model.save(filename=model.model_name, directory=self.model_dir)
...
#     # def store_model(self, model):
#     # # Save only the model weights using model's own save method
#     #     model.save(filename=model.model_name, directory=self.model_dir)

#     # ------------------------------- LOADING -------------------------------

#     #def network_load_weights(self, network, model_dir, stage, episode, name):
#         #filepath = os.path.join(model_dir, str(network.name) + '_stage'+str(stage)+'_episode'+str(episode)+'.pt')
...
#     def network_load_weights(self, network, model_dir, stage, episode, name):
# 	    filepath = os.path.join(model_dir, name + '_stage' + str(stage) + '_episode' + str(episode) + '.pt')
# 	    print(f"loading: {name} model from file: {filepath}")
# 	    network.load_state_dict(torch.load(filepath, self.map_location))
...
#         for name, network in networks.items():
#             self.network_load_weights(network, self.session_dir, self.stage, self.load_episode, name)
