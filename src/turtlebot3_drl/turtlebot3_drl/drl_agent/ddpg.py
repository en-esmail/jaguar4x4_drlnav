...
class DDPG(OffPolicyAgent):
    ...
    def get_action(self, state, is_training, step, visualize=False):
        ...
        # Only add noise if training AND noise object exists
        if is_training and hasattr(self, "noise") and self.noise is not None:
            try:
                noise = torch.from_numpy(copy.deepcopy(self.noise.get_noise(step))).to(self.device)
                action = torch.clamp(torch.add(action, noise), -1.0, 1.0)
            except Exception as e:
                print("⚠️ Warning: Noise could not be applied, running without noise:", e)
