...
class DrlAgent(Node):
    def __init__(self, training, algorithm, load_session="", load_episode=0, real_robot=0):
        ...
        if self.algorithm == 'dqn':
            ...
            self.model = SACAgent(state_dim, action_dim, self.device, max_action,buffer_size=1000000)
    ...
    def process(self):
        ...
        while (True):
            ...
            while not episode_done:
                ...
                # >>> Inject action noise if training and algorithm needs it
                if self.training and hasattr(self, "noise") and self.noise is not None:
    ...
    def finish_episode(self, step, eps_duration, outcome, dist_traveled, reward_sum, loss_critic, lost_actor):
            ...
            print(f"steps: {step:<6}steps_total: {self.total_steps:<7}dist: {dist_traveled:<6.2f}time: {eps_duration:<6.2f}")
...
def main_train_real(args=sys.argv[1:]):
    # Force training mode ON (1) and real_robot ON (1)
    args = ['1'] + args + ['1']
    main(args)
