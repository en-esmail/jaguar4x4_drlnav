...
class DiagGaussianActor(nn.Module):
    ...
    #     return dist
    def forward(self, obs, visualize=False):
        mu, log_std = self.trunk(obs).chunk(2, dim=-1)
        ...
        if visualize and hasattr(self, 'visual') and self.visual:
            # assuming trunk is a sequential of layers
            intermediate = obs
            activations = []
            for layer in self.trunk[:-1]:  # skip final layer if needed
                intermediate = layer(intermediate)
                activations.append(intermediate)

            self.visual.update_layers(obs, mu, activations, [p.bias for p in self.trunk if hasattr(p, 'bias')])
