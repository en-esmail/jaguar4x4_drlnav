...
class DRLEnvironment(Node):
    def __init__(self):
        ...
        # self.raw_cmd_pub = self.create_publisher(String, 'ros_drrobot_motor_cmd', 10)
    ...
    def step_comm_callback(self, request, response):
        ...
        if not hasattr(self, 'estop_released'):
            self.estop_released = False

        # Release E-STOP before sending any motor commands
        if not self.estop_released:
            msg = String()
            msg.data = "MMW !MG"      # Release E-STOP
            self.raw_cmd_pub.publish(msg)
            self.get_logger().info("DRL: Sending E-STOP Release Command (MMW !MG)")
            self.estop_released = True

            # Don't move on the same step; return safe state 
            return self.initalize_episode(response)
        # ---------------------------------------------------------

        # # --- 1) Release E-stop / enable motors ---
        # release = String()
        # release.data = "MMW !MG"      # or "MMW !EX" depending on your firmware
        # self.raw_cmd_pub.publish(release)
        ...
        # Modified on 14/11 to be suitable for jaguar robot 4 wheel# ------------------------------------
        # Convert (v, w) to skid-steer wheel speeds
        # ------------------------------------
        WHEEL_BASE = 0.52  # meters, adjust for your robot

        left_speed  = final_linear - final_angular * (WHEEL_BASE / 2.0)
        right_speed = final_linear + final_angular * (WHEEL_BASE / 2.0)

        # Convert m/s → PWM (-400 to 400)
        MAX_PWM = 400.0
        MAX_LINEAR = REAL_SPEED_LINEAR_MAX   # from settings

        left_pwm  = int((left_speed / MAX_LINEAR) * MAX_PWM)
        right_pwm = int((right_speed / MAX_LINEAR) * MAX_PWM)

        # Clip to allowed range
        left_pwm  = max(-400, min(400, left_pwm))
        right_pwm = max(-400, min(400, right_pwm))

        # Create raw motor command string
        cmd = f"MMW !M {left_pwm} {right_pwm}"

        msg = String()
        msg.data = cmd
        self.raw_cmd_pub.publish(msg)   # <-- FIXED
        ...
        if self.done:
                # --- SEND E-STOP WHEN EPISODE IS DONE ---
            stop = String()
            stop.data = "MMW !EX"
            self.raw_cmd_pub.publish(stop)
            self.get_logger().warn("Episode done → Sending E-STOP (MMW !EX)")
            # ---------------------------------------------------------------
            self.new_goal = False
